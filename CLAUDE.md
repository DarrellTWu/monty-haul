...
- **Stash mutations** (`client/src/store/stash.js` → `server/routes/hub.js` → `server/store/playerStore.js`): `buyItem(id, price)`, `sellItem(id, price)`, `craftRecipe(recipe)`, `dumpRaiderPackToStash()`. All async, return `Promise<bool>`. Server validates and applies atomically; client cache updated on success. Caller passes prices computed via `sellPrice()` / `VENDOR_CATALOG` lookup.
