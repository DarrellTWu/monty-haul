...
| `systems/MovementSystem.js` | `update(state, dt, bounds, geometry, enemyDefs)`: integrates velocity → `resolveWallCollision` → `tryAutoClimb` → clamp to bounds. Speed = base + longstrider + Unarmored Movement (monk 2+, no armor/shield), ×2 while `'dash'` (Step of the Wind / Cunning Action), ×0.5 while `'climb_fatigue'`. Ticks `cunningActionCooldownMs` alongside the attack timer. Returns climb-fatigue events (underleveled perimeter-wall climbs) for DungeonRoom to convert into conditions. Ends with the body-blocking pass: pairwise `separateCircles` on living same-elevation entities + wall re-resolution. Player `canClimb` from `getDerivedClassFeatures`; enemy `canClimb` from `enemyDefs`. |
