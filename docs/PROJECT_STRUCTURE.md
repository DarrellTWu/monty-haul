...
status: shipped
updated: 2026-07-13
purpose: Canonical file-layout reference. Source of truth — CLAUDE.md and tech_spec.md link here, do not duplicate. Last bump: Rogue class + Skirmisher subclass (Sneak Attack, Cunning Action, skirmisher_spurs emblem).
---
...
| `state/PlayerState.js` | Position, velocity, HP/AC, class (primary), equipment slots, inventory, hotbar, conditions, ability scores, rage tracking, `gold`, `kills`, `elevation`. Class resources: `secondWindAvailable`, `actionSurgeAvailable`, `kiPoints`/`kiMax`, `cunningActionCooldownMs`; condition mirrors incl. `patientDefenseRemainingMs`/`dashRemainingMs`. Level-up: `classLevels` (MapSchema), `levelUpHistory` (ArraySchema), `subclasses` (MapSchema classId→subclassId), `pendingLevelUp` boolean, cached `level` (mutated only by `applyClassLevel`). |
...
| `shared/tests/class-progression.test.js` | 28 | `applyClassLevel` invariant + resource seeding (action surge, ki, rage scaling), same-class eligibility + cap, derived features incl. subclass grants, `trySubclassUnlock` paths, `getSneakAttackDice` scaling + Skirmisher unlock. |
