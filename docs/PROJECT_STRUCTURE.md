...
status: shipped
updated: 2026-07-13
purpose: Canonical file-layout reference. Source of truth — CLAUDE.md and tech_spec.md link here, do not duplicate. Last bump: level 1–3 progression + subclass emblems, Sprint C (rooms/CORS/floor validation), Sprint D (auth), kill attribution, unlock conditions, balance sim.
---
...
| `store/playerStore.js` | Async write-through cache backed by Supabase. In-memory `Map<playerId, state>` is fast path; miss → `loadPlayer`/`loadPlayerByUsername`. `authenticate(username, password)` — register / legacy-adopt / verify (see `agent-context/persistence.md`). Every mutation modifies cached state then awaits `syncStashAndMeta`. Per-player mutation lock (`_withLock`). Server-authoritative pricing. `commitExtract`/`commitDeath` dead-letter on persistence failure. |
| `systems/CombatSystem.js` | Wraps `shared/logic/combat.js`. Advantage/disadvantage sources assembled per call site (high-ground, reckless, patient-defense, long-range, foe-adjacent). Derived class features drive Dueling, `critRange` (Champion), Frenzy extra attack, Martial Arts bonus strike (any monk level). Exports `applySecondWind` (1d10 + fighter class level), `applyActionSurge`, `applyFlurryOfBlows` (2 unarmed strikes, 1 ki; Open Hand stagger). Increments `player.kills` at every enemy-death site. |
| `systems/MovementSystem.js` | `update(state, dt, bounds, geometry, enemyDefs)`: integrates velocity → `resolveWallCollision` → `tryAutoClimb` → clamp to bounds. Speed = base + longstrider + Unarmored Movement (monk 2+, no armor/shield), ×2 while `'dash'` (Step of the Wind). Player `canClimb` from `getDerivedClassFeatures`; enemy `canClimb` from `enemyDefs`. |
...
| `state/PlayerState.js` | Position, velocity, HP/AC, class (primary), equipment slots, inventory, hotbar, conditions, ability scores, rage tracking, `gold`, `kills`, `elevation`. Class resources: `secondWindAvailable`, `actionSurgeAvailable`, `kiPoints`/`kiMax`; condition mirrors incl. `patientDefenseRemainingMs`/`dashRemainingMs`. Level-up: `classLevels` (MapSchema), `levelUpHistory` (ArraySchema), `subclasses` (MapSchema classId→subclassId), `pendingLevelUp` boolean, cached `level` (mutated only by `applyClassLevel`). |
...
| `data/enemies/tier1.js` | Goblin, dog, skeleton + `ENEMY_REGISTRY` (type → stat block; used by `_loadFloor` and the boot validator). Each carries `canClimb: bool`. |
| `data/classes/{fighter,barbarian,monk,index}.js` | `CLASS_REGISTRY`. Each class carries `levels: { [n]: { features, grants } }` filled for 1–3, `subclasses` (Champion / Berserker / Open Hand), `startingItemIds` (emblem), `gearlessLevelCap: 3`, `name`, passive flags `canClimb`/`unarmoredDefense` ('wis' Monk, 'con' Barbarian), Barbarian `rageUses`. |
...
| `tests/{combat,loot,geometry,character,equipment,conditions,items,class-progression,floors}.test.js` | Pure-logic unit tests + itemization/floor validators. |
...
| File | Count | Notes |
|---|---|---|
| `shared/tests/combat.test.js` | 51 | Advantage/disadvantage cancellation, `pickAttackMode` dispatch, expanded crit range (Champion 19 — crit-but-not-auto-hit), `resolveSave` advantage (Danger Sense). |
...
| `shared/tests/class-progression.test.js` | 24 | `applyClassLevel` invariant + resource seeding (action surge, ki, rage scaling), same-class eligibility + cap, derived features incl. subclass grants, `trySubclassUnlock` paths. |
| `shared/tests/items.test.js` | 109 | Itemization validator — base shape, per-category fields (incl. emblem `unlocks` integrity), registry-key parity, disjoint namespaces, `getItemDisplay` completeness, reference integrity for chests/loot/vendors/recipes, recipe bench reference. |
