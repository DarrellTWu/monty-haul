...
| `use_hotbar` | `{ slot }` | Activate hotbar slot 0–9. Dropped while `pendingLevelUp` (same action lock as `move`/`attack`). Abilities dispatch via `DungeonRoom._useAbility` and require the feature in `getGrantedFeatures(player)` (binding alone isn't enough): `second_wind`, `rage`, `action_surge`, `reckless_attack` (toggle), `flurry_of_blows` / `patient_defense` / `step_of_wind` (1 ki each), `cunning_action`. Consumable types: `healing`, `bless`, `longstrider`, `false_life`, `extract` (run terminator) |
...
| `extract_committed` | `{ ok }` | Sent **only to the extracting client** once `commitExtract` settles. Drives the run summary's save-status line ("Saving run to vault…" → confirm/warn; client times out to a warning after `EXTRACT_ACK_TIMEOUT_MS` = 8 s). `ok: false` means persistence failed after retries — the payload is in the dead-letter queue for operator replay. No-op if the client already left. |
...
**Payload shape is attacker-controlled.** Every handler coerces its payload through `asObj()` before destructuring (null/primitive/array payloads degrade to no-ops), `move` rejects non-finite `dx`/`dy` (NaN would poison position integration), and hotbar slots validate as integers 0–9 (`asSlotIndex`). `DungeonRoom.onUncaughtException` is the backstop: Colyseus 0.15 only try/catches handlers (and the simulation tick) when it's defined — without it a handler throw kills the whole Node process. When adding a handler: destructure via `asObj(payload)`, validate numerics with `Number.isFinite`/`Number.isInteger`, and rely on the backstop never. `server/tests/payload-hardening.test.js` fires hostile payloads at every registered message type and fails if any handler reaches the backstop.

Open gap: server-side ability score validation only checks keys exist, not budget/range. See architecture review §3.2.

## HTTP (Express `/hub`)
Router at `server/routes/hub.js`. CORS: allowlist from `ALLOWED_ORIGINS` env (comma-separated); unset → wildcard for local dev. `Access-Control-Allow-Headers` must include `Authorization` (browsers preflight it on every authenticated route).
...
| Route | Body | Returns |
|---|---|---|
| `POST /login` | `{ username, password }` (password ≥ 6 chars; unknown username registers) | `{ token, playerId, username, stash, gold, raiderPack }`; 401 `invalid_credentials` on mismatch; 400 `invalid_username` (shared `validateUsername`: trimmed, non-empty, ≤ `USERNAME_MAX_LENGTH` = 20 — same rule as rename). A registration race (two first-time logins, same name) resolves as a normal login attempt for the loser, not a 500. |
